        ...
        @foreach ($posts as $post)
            <div class="blog_left post_show post_index">
                <div class="post-head">
                        ...
                        <h3 class="info">Posted {{ $post->created_at->diffForHumans() }} in <a href="/categories/{{ $post->category->name }}">{{ $post->category->name }}</a> By <a href="/users/{{ $post->user->name }}">{{ $post->user->name }}</a>.</h3>
                        <div class="btn-post">
